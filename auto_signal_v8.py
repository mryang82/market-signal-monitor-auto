...
def fetch_fear_greed():
    """CNN Fear & Greed Index (주식 sentiment) - alternative.me는 비트코인 fallback
    
    GitHub Actions 환경에서 CNN의 비공식 API에 접근 (production.dataviz.cnn.io).
    
    반환 DataFrame 호환성:
      - 컬럼 'value': float/int (최신값)
      - iloc[0]['value']: 가장 최근값
      - iloc[N]['value']: N일 전 값 (대략)
      - 호출자가 int() 변환하므로 float도 OK
    """
    from datetime import datetime, timedelta
    import pandas as pd
    
    # 1차 시도: CNN F&G (주식 sentiment, 정확)
    try:
        ...
        if r.status_code == 200:
            ...
            if len(hist) > 0:
                ...
                # x = unix timestamp(ms), y = F&G 값
                df["timestamp_ms"] = pd.to_numeric(df["x"])
                df["date"] = pd.to_datetime(df["timestamp_ms"], unit="ms").dt.normalize()
                df["value"] = pd.to_numeric(df["y"]).round().astype(int)  # int 변환 (호환성)
                
                # 최신순 정렬
                df = df.sort_values("date", ascending=False).reset_index(drop=True)
                
                # 중복 날짜 제거 (CNN이 가끔 동일 날짜 다른 값 보냄)
                df = df.drop_duplicates(subset="date", keep="first").reset_index(drop=True)
                
                # 30일 전 값을 정확한 날짜로 찾기 (인덱스가 아니라)
                target_1m = df.iloc[0]["date"] - pd.Timedelta(days=30)
                idx_1m = (df["date"] - target_1m).abs().idxmin()
                
                # iloc[29] 호환을 위해 인덱스 조정
                # 호출자가 fg.iloc[min(29, len(fg)-1)] 호출하므로
                # idx_1m 위치로 데이터 재정렬 가능
                # 간단히는 그대로 두고 정렬만 보장
                
                print(f"  ✓ CNN F&G (주식): 최신값 {df.iloc[0]['value']} (날짜 {df.iloc[0]['date'].date()})")
                if len(df) > idx_1m:
                    print(f"      30일 전: {df.iloc[idx_1m]['value']} (날짜 {df.iloc[idx_1m]['date'].date()})")
                
                return df[["value", "date"]]
    except Exception as e:
        print(f"   ⚠️ CNN F&G 실패: {e}")
    
    # 2차 fallback: alternative.me (⚠️ 비트코인 sentiment! 주식과 다름)
    print("   ⚠️ CNN 실패 → alternative.me fallback (주의: 비트코인 sentiment, 주식과 다름)")
    try:
...
def collect_data():
    ...
    if fg is not None:
        ...
        # 1개월 전 값: 'date' 컬럼 있으면 정확한 날짜 lookup, 없으면 인덱스 기반 (fallback)
        if "date" in fg.columns:
            target_1m = fg.iloc[0]["date"] - pd.Timedelta(days=30)
            idx_1m = (fg["date"] - target_1m).abs().idxmin()
            v1mago = int(fg.loc[idx_1m, "value"])
        else:
